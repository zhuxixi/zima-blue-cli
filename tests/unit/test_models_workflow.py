...
from zima.models.workflow import (
    VariableDef,
    WorkflowConfig,
)
...
class TestVariableDef(TestIsolator):
    ...
    class TestValidation:
        ...
        @pytest.mark.parametrize(
            "invalid_type", ["invalid", "text", "int", "float", "list", "dict"]
        )
...
class TestWorkflowConfig(TestIsolator):
    ...
    class TestCreate:
        ...
        def test_create_basic(self):
            ...
            workflow = WorkflowConfig.create(
                code="test-workflow", name="Test Workflow", template="Hello {{ name }}!"
            )
        ...
        def test_create_with_variable_objects(self):
            ...
            workflow = WorkflowConfig.create(
                code="var-obj-workflow", name="Variable Object Workflow", variables=[var1, var2]
            )
        ...
        def test_create_invalid_format(self):
            ...
            with pytest.raises(ValueError) as exc_info:
                WorkflowConfig.create(code="test", name="Test", format="invalid-format")
    ...
    class TestValidation:
        ...
        def test_validate_valid(self):
            """Test valid workflow."""
            workflow = WorkflowConfig.create(code="valid-workflow", name="Valid Workflow")
        ...
        def test_validate_invalid_code_format(self):
            ...
            workflow = WorkflowConfig.create(code="Invalid_Code", name="Test")
            errors = workflow.validate()
        ...
        def test_validate_variable_errors(self):
            ...
            workflow = WorkflowConfig.create(
                code="test", name="Test", variables=[{"name": "", "type": "invalid-type"}]
            )
            errors = workflow.validate()
            assert any("name is required" in e for e in errors)
            assert any("Invalid variable type" in e for e in errors)
    ...
    class TestVariableValidation:
        ...
        def test_validate_nested_paths(self):
            ...
            errors = workflow.validate_variables(
                {"task": {"name": "Test"}, "config": {"timeout": 30}}
            )
            assert errors == []

            errors = workflow.validate_variables(
                {"task": {"name": ""}, "config": {}}  # Empty string is still a value, not None
            )
    ...
    class TestTagManagement:
        ...
        def test_remove_tag(self):
            """Test removing tags."""
            workflow = WorkflowConfig.create(code="test", name="Test", tags=["tag1", "tag2"])
