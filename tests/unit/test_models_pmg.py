...
from zima.models.pmg import (
    PMGConfig, ParameterDef, ExtendDef, ConditionDef, ConditionEvaluator,
    VALID_PARAM_TYPES, VALID_PMG_FOR_TYPES
)
...
class TestParameterDef(TestIsolator):
    ...
    class TestRender:
        ...
        def test_render_key_value(self):
            ...
            param = ParameterDef(name="labels", type="key-value", value={"env": "prod", "team": "platform"})
            result = param.render()
...
class TestConditionEvaluator(TestIsolator):
    ...
    def test_evaluate_or(self, monkeypatch):
        """Test OR condition."""
        result = ConditionEvaluator.evaluate("os == 'invalid1' || os == 'invalid2' || os != 'never'")
        assert result is True
        ...
        def test_from_dict(self):
            ...
            data = {
                ...
                "spec": {
                    "forTypes": ["claude"],
                    "parameters": [
                        {"name": "model", "type": "long", "value": "claude-3"}
                    ],
                    "raw": "--test",
                    "extends": [{"code": "base"}]
                }
