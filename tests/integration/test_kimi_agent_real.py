...
pytestmark = pytest.mark.skipif(
    not check_kimi_available(),
    reason="kimi-cli not available or not authenticated"
)
...
class TestKimiAgentRealCommands:
    ...
    def test_kimi_cli_simple_print_mode(self):
        ...
        prompt_file.write_text(
            "Respond with a JSON object: {\"status\": \"ok\", \"message\": \"Hello\"}",
            encoding="utf-8"
        )
...
"""
    ...
    def test_agent_config_to_real_command(self):
        ...
        prompt_file.write_text("Say 'Hello from test' and output JSON with status=completed", encoding="utf-8")
...
""",
    ...
    def test_pjob_render_then_execute(self):
        ...
        render_result = runner.invoke(app, [
            "pjob", "render", "real-execution-pjob"
        ])
    ...
    def test_invalid_work_directory(self):
        ...
        result = subprocess.run(
            [
                ...
                "--print",
                "--prompt", str(prompt_file),
                "--work-dir", str(invalid_workspace),
                "--max-steps-per-turn", "1",
            ],
            capture_output=True,
            text=True,
