...
class TestKimiAgentRunnerLayer:
    ...
    def test_kimi_runner_directory_setup(self):
        ...
        KimiRunner(self.agent, self.agent_dir)
...
"""
        ...
        result = runner.run_cycle(
            prompt="Test prompt for execution", cycle_num=1, task_name="Test Task"
        )
    ...
    def test_kimi_runner_timeout_handling(self, mock_run):
        ...
        mock_run.side_effect = subprocess.TimeoutExpired(
            cmd=["kimi"], timeout=self.agent.max_execution_time
        )
        ...
        # Run cycle
        result = runner.run_cycle(prompt="Test prompt", cycle_num=2, task_name="Timeout Test")
...
"""
        ...
        # Run cycle
        result = runner.run_cycle(prompt="Test prompt", cycle_num=3, task_name="Failure Test")
...
class TestKimiAgentCLILayer:
    ...
    def test_pjob_run_with_kimi_agent_dry_run(self):
        ...
        result = runner.invoke(app, ["pjob", "run", "test-kimi-pjob", "--dry-run"])
...
class TestKimiAgentEdgeCases:
    ...
    def setup_isolation(self, monkeypatch, tmp_path):
        ...
        self.agent = AgentConfig.create(
            code="test-kimi-edge", name="Edge Case Agent", agent_type="kimi"
        )
...
"""
        ...
        for i in range(5):
            result = runner.run_cycle(
                prompt=f"Prompt for Cycle {i+1}", cycle_num=i + 1, task_name=f"Task {i+1}"
            )
        ...
        assert (
            len(set(prompt_files)) == 5
        ), f"Prompt files should have unique paths, got: {prompt_files}"
