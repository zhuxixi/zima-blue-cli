...
9. **PR + Zima 单 Bot CR + 监听** → **REQUIRED SUB-SKILL: Use zima-pr-monitor**（开 PR、打 `zima:needs-review`、轮询、解析 review meta（`cc-cr-meta` / `pi-cr-meta` 前缀区分；`kimi-cr-meta` 忽略）、worktree 修、重打标签、收敛判定、合并）
