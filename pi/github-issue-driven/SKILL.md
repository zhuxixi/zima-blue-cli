...
处理 GitHub issue 的标准研发闭环：issue → 调研 → 设计 → 实现 → CR → 合并。JFox 知识库是调研中枢，superpowers 处理设计与实现，Zima 双 Bot CR 把关合并。
...
9. **PR + Zima 双 Bot CR + 监听** → **REQUIRED SUB-SKILL: Use zima-pr-monitor**（开 PR、打 `zima:needs-review`、轮询、解析双 bot 结果、worktree 修、重打标签、收敛判定、合并）
