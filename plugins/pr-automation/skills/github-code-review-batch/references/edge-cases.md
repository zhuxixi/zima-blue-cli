...
过滤原因（#124）：CLAUDE.md 中并未明确规定函数命名的具体要求，issue-validator 无法定位到具体规则，按规范类收紧策略标记 `valid: false`。规范类 issue 必须可在 CLAUDE.md/AGENTS.md 中定位到具体规则（原文片段或章节），否则过滤。
...
- **缺失导入 / 未解析引用**：由确定性 tool-layer（`tsc` / `mypy` / `ruff` 等，#121）零误报覆盖；LLM agent 不再重复报告，与下方"不标记 Linter/TypeChecker 可捕获的问题"一致
