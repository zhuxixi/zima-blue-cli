...
@app.command()
def list(
    for_workflow: Optional[str] = typer.Option(None, "--for-workflow", "-w", help="Filter by workflow code"),
    ...
    if not configs:
        console.print("[yellow]No variable configs found.[/yellow] Create one with: zima variable create")
        return
    ...
    if for_workflow:
        configs = [
            c for c in configs 
            if c.get("spec", {}).get("forWorkflow") == for_workflow
        ]
...
def set(
    code: str = typer.Argument(..., help="Variable config code"),
    key: str = typer.Option(..., "--key", "-k", help="Variable key (dot notation supported, e.g., 'task.name')"),
    value: str = typer.Option(..., "--value", "-v", help="Variable value (JSON format for complex values)"),
):
...
def validate(
    code: str = typer.Argument(..., help="Variable config code"),
    check_workflow: bool = typer.Option(False, "--check-workflow", help="Validate against target workflow"),
):
