...
from zima.models.env import EnvConfig, SecretDef, VALID_ENV_FOR_TYPES
...
def list(
    ...
    if for_type:
        configs = [
            c for c in configs 
            if c.get("spec", {}).get("forType") == for_type
        ]
...
def show(
    ...
    resolve_secrets: bool = typer.Option(False, "--resolve-secrets", help="Resolve and show secret values (use with caution)"),
):
...
def update(
    ...
    override_existing: Optional[bool] = typer.Option(None, "--override-existing", help="Update override existing setting"),
):
...
def set(
    ...
    value: Optional[str] = typer.Option(None, "--value", "-v", help="Variable value (not needed for secrets)"),
    secret: bool = typer.Option(False, "--secret", "-s", help="Store as secret (don't save value directly)"),
    source: str = typer.Option("env", "--source", help="Secret source: env/file/cmd"),
    source_key: Optional[str] = typer.Option(None, "--source-key", help="Source key (for env source)"),
    source_path: Optional[str] = typer.Option(None, "--source-path", help="Source path (for file source)"),
    source_cmd: Optional[str] = typer.Option(None, "--source-cmd", help="Source command (for cmd source)"),
):
        ...
        if secret:
            ...
            if source == "env":
                if not source_key and not value:
                    console.print(f"[red]✗[/red] --source-key or --value is required for env source")
                    raise typer.Exit(1)
                if not source_key:
                    source_key = value
            
            if source == "file" and not source_path:
                console.print(f"[red]✗[/red] --source-path is required for file source")
                raise typer.Exit(1)
            
            if source == "cmd" and not source_cmd:
                console.print(f"[red]✗[/red] --source-cmd is required for cmd source")
                raise typer.Exit(1)
            ...
            if value is None:
                console.print(f"[red]✗[/red] --value is required for plain variables")
                raise typer.Exit(1)
...
def export(
    ...
    resolve_secrets: bool = typer.Option(False, "--resolve-secrets", help="Resolve secret values (use with caution)"),
):
