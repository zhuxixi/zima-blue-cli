...
def list_pjobs(
    ...
    if label:
        configs = [
            c for c in configs
            if any(l in c.get("metadata", {}).get("labels", []) for l in label)
        ]
    ...
    try:
        ...
        if remove_label:
            config.metadata.labels = [
                l for l in config.metadata.labels
                if l not in remove_label
            ]
...
def run(
    ...
    set_var: Optional[List[str]] = typer.Option(None, "--set-var", help="Override variable values (key=value)"),
    set_env: Optional[List[str]] = typer.Option(None, "--set-env", help="Override environment variables (key=value)"),
    set_param: Optional[List[str]] = typer.Option(None, "--set-param", help="Override agent parameters (key=value)"),
...
def render(
    code: str = typer.Argument(..., help="PJob code"),
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Save rendered prompt to file"),
    show_command: bool = typer.Option(False, "--show-command", help="Show the command that would be executed"),
