...
from zima.models.pmg import VALID_PARAM_TYPES, VALID_PMG_FOR_TYPES, ParameterDef, PMGConfig
...
def create(
    ...
    if manager.config_exists("pmg", code):
        if force:
            ...
            console.print(
                f"   Use [bold]--force[/bold] to overwrite, or [bold]zima pmg update {code}[/bold] to modify"
            )
            raise typer.Exit(1)
    ...
    try:
        config = PMGConfig.create(
            code=code, name=name, for_types=for_types, description=description
        )
...
def list(
    ...
    if for_type:
        configs = [c for c in configs if for_type in c.get("spec", {}).get("forTypes", [])]
...
def add_param(
    ...
    values: Optional[str] = typer.Option(
        None, "--values", help="Multiple values (comma-separated, for repeatable type)"
    ),
    enabled: bool = typer.Option(
        True, "--enabled/--disabled", help="Whether flag is enabled (for flag type)"
    ),
):
    ...
    if param_type == "flag":
        ...
        if not values:
            console.print("[red]✗[/red] --values is required for repeatable type")
            raise typer.Exit(1)
    ...
    try:
        param = ParameterDef(
            name=name, type=param_type, value=param_value, values=param_values, enabled=enabled
        )
...
def build(
    ...
    eval_conditions: bool = typer.Option(
        True, "--eval-conditions/--no-eval-conditions", help="Evaluate conditions"
    ),
):
