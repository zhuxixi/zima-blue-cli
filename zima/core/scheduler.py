...
class CycleScheduler:
    ...
    def __init__(self, config: AgentConfig, runner: KimiRunner, state_manager: StateManager):
        self.config = config
        self.runner = runner
    ...
    def run(self) -> None:
        ...
        while self.running:
            ...
            safe_print(f"\n{'='*60}")
            safe_print(
                f"{icon('cycle')} Cycle {cycle_num} started - {cycle_start.strftime('%H:%M:%S')}"
            )
            safe_print(f"{'='*60}")
            ...
            try:
                ...
                with open(agent_state_file, "r", encoding="utf-8") as f:
                    ...
                    if progress.get("pending_files"):
                        project_context += (
                            f"- **Pending Test Files**: {len(progress['pending_files'])}\n"
                        )
                    if progress.get("completed_files"):
                        project_context += (
                            f"- **Completed Files**: {len(progress['completed_files'])}\n"
                        )
            except Exception:
                pass
...
"""
    ...
    def _handle_result(self, result: CycleResult, agent_state: AgentState, task: dict) -> None:
        ...
        if result.status == "completed":
            ...
            if current_retries < self.config.max_retries:
                ...
                safe_print("   Max retries reached, marking as failed")
