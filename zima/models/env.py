...
class SecretResolver:
    ...
    def _resolve_env(secret: SecretDef) -> str:
        ...
        if value is None:
            raise ValueError(
                f"Secret '{secret.name}': Environment variable '{secret.key}' not set"
            )
        return value
    ...
    def _resolve_file(secret: SecretDef) -> str:
        ...
        if not file_path.exists():
            raise ValueError(
                f"Secret '{secret.name}': File not found: {file_path}"
            )
...
class EnvConfig(BaseConfig):
    ...
    def create(
        ...
        if for_type not in VALID_ENV_FOR_TYPES:
            raise ValueError(
                f"Invalid for_type: {for_type}. Valid: {VALID_ENV_FOR_TYPES}"
            )
    ...
    def validate(self) -> list[str]:
        ...
        if not self.for_type:
            ...
            errors.append(f"spec.forType '{self.for_type}' is not valid. Valid: {VALID_ENV_FOR_TYPES}")
    ...
    def set_secret(
        ...
        if name in self.variables:
            raise ValueError(f"Key '{name}' already exists as a variable. Use unset_variable first.")
        
        # Remove existing secret with same name
        self.secrets = [s for s in self.secrets if s.name != name]
        
        # Add new secret
        self.secrets.append(SecretDef(
            name=name,
            source=source,
            key=key,
            path=path,
