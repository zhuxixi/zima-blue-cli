...
class ConditionDef:
    ...
    def to_dict(self) -> dict:
        """Convert to dictionary."""
        return {"when": self.when, "parameters": [p.to_dict() for p in self.parameters]}
    ...
    def from_dict(cls, data: dict) -> ConditionDef:
        ...
        return cls(when=data.get("when", ""), parameters=parameters)
...
class ConditionEvaluator:
    ...
    def _build_context() -> dict:
        ...
        return {"os": os_map.get(system, system), "arch": arch_map.get(machine, machine)}
...
class PMGConfig(BaseConfig):
    ...
    def build_command(
        self, eval_conditions: bool = True, loaded_pmgs: Optional[dict[str, "PMGConfig"]] = None
    ) -> list[str]:
        """
        ...
        if loaded_pmgs:
            for extend in self.extends:
                ...
                if parent:
                    parent.build_command(eval_conditions=False, loaded_pmgs=loaded_pmgs)
