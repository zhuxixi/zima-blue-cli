...
class WorkflowConfig(BaseConfig):
    ...
    def create(
        ...
        if format not in VALID_TEMPLATE_FORMATS:
            raise ValueError(
                f"Invalid template format: {format}. " f"Valid formats: {VALID_TEMPLATE_FORMATS}"
            )
    ...
    def get_template_completeness(self) -> dict[str, any]:
        ...
        result["has_all_required_sections"] = required_count == len(
            STANDARD_AGENT_SECTIONS["required"]
        )
